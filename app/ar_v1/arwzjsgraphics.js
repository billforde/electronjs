...
*
* _Name_        ===> ar_v1/arwzjsgraphics.js
* _Description_ ===>
*
* _History_:
...
ActiveJSRevision["arwzjsgraphics"]="$Revision: 20160610.1256 $";
...
// Under XUL, owner of 'document' must be specified explicitly
{
    ...
    jg_fast=false;//fix issue in ie9
...
function _mkDiv(x, y, w, h)
{
    this.htm += '<div style="position:absolute;'+
        ...
        (this.clickOn?'cursor:pointer;':'')+
        'background-color:' + this.color +
        (!jg_moz? ';overflow:hidden' : '')+
        ';"'+
        (this.title?' title="'+this.title+'"':'')+
        (this.clickOn?' onclick="'+this.clickOn+'"':'')+
        '><\/div>';
}
...
interior_extrema. */
    ...
    this.drawString = function(txt, x, y)
    {
        this.htm += '<div style="position:absolute;white-space:nowrap;'+
            ...
            'font-size:' + this.ftSz + 'pt;'+
            'color:' + this.color + ';' + this.ftSty + '"' +(this.title?' title="'+this.title+'"':'')+
            (this.clickOn?' onclick="'+this.clickOn+'"':'')+'>'+
            txt +
            '<\/div>';
    };

/* drawStringRect() added 
...
/* drawStringClipRect() added
Allows to specify the size of and clip to the text rectangle and to align the
text both horizontally (e.g. right) and vertically within that rectangle */
    this.drawStringClipRect = function(txt, x1, y1, width1, halign, tooltip)
    {
        var x = parseInt(x1);
        var y = parseInt(y1);
        var width = parseInt(width1);
        this.htm += '<div '+ ((tooltip != null)? 'title="' + txt + '" ':'')+
            'style="position:absolute;white-space:nowrap;overflow:hidden;'+
            'left:' + x + 'px;'+
            'top:' + y + 'px;'+
            'width:'+width +'px;'+
            //((jg_ie) ? 'clipRight:' + (x+width) + 'px;' : '') +
            'text-align:'+halign+';'+
            'font-family:' +  this.ftFam + ';'+
            'font-size:' + this.ftSz + 'pt;'+
            'color:' + this.color + ';' + this.ftSty + '"'+ (this.title?' title="'+this.title+'"':'')+
